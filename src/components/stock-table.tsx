...
export const StockTable = ({ stocks, loading, lastUpdated }: StockTableProps) => {
		...
		return (
			<div className="flex flex-col">
				<span className={`font-semibold ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
					{isPositive ? '+' : ''}{rowData.change.toFixed(2)}
				</span>
				<span className={`text-xs ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
					{isPositive ? '+' : ''}{rowData.changePercent.toFixed(2)}%
				</span>
			</div>
		)
	}
	...
	const exportCSV = () => {
		const csvContent = [
			['Symbol', 'Name', 'Price', 'Change', 'Change %', 'Volume', 'Last Updated'],
			...stocks.map(stock => [
				stock.symbol,
				stock.name,
		...
		a.href = url
		a.download = `market-mage-portfolio-${new Date().toISOString().split('T')[0]}.csv`
		a.click()
	...
	return (
		<div className="card">
			<DataTable
				...
				showGridlines
			>
				<Column field="symbol" header="Symbol" sortable style={{ minWidth: '100px' }} />
				<Column field="name" header="Company" sortable style={{ minWidth: '150px' }} />
				<Column field="price" header="Price" body={priceBodyTemplate} sortable style={{ minWidth: '100px' }} />
				<Column field="change" header="Change" body={changeBodyTemplate} sortable style={{ minWidth: '120px' }} />
				<Column field="volume" header="Volume" body={volumeBodyTemplate} sortable style={{ minWidth: '120px' }} />
