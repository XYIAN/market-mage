...
import { CryptoDashboardStepper } from './CryptoDashboardStepper'
import {
...
export function CustomizableCryptoDashboard() {
  const [config, setConfig] = useState<CryptoDashboardConfig | null>(null)
  ...
  const saveDashboardConfig = (newConfig: CryptoDashboardConfig) => {
    if (typeof window === 'undefined') return
    try {
  ...
  const handleSaveConfig = async (newConfig: CryptoDashboardConfig) => {
    setSaving(true)
  ...
  const handleStepperSave = async (newConfig: CryptoDashboardConfig) => {
    setSaving(true)
      ...
      case 'ai-oracle':
        return (
          <CryptoAIOracle
            key={section.id}
            assets={config?.assets || []}
            refreshCount={config?.aiOracleRefreshCount || 0}
            onRefreshCountChange={handleAIOracleRefresh}
            lastRefresh={config?.lastAiOracleRefresh}
          />
        )
      case 'insights':
        return <CryptoInsights key={section.id} assets={config?.assets || []} />
  ...
  return (
    <div className="min-h-screen p-4">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              {config.assets.length} assets • {enabledSections.length} features
            </p>
          </div>
          <Button
      ...
      <DashboardCustomizationDialog
        visible={showCustomizeDialog}
        onHide={() => setShowCustomizeDialog(false)}
